...
Логически и функционально независимый компонент страницы. Если у блока есть DOM-представление, осуществляет инкапсуляцию своих данных в [DOM-дереве](https://ru.wikipedia.org/wiki/Document_Object_Model), что позволяет скрыть внутреннюю реализацию блока и обеспечить его независимую работу. Взаимодействие блоков между собой происходит с помощью API.
...
Если внешний вид блока задан с помощью CSS, это означает, что блок реализован в технологии CSS. А если документация к блоку написана в формате MarkDown, то — блок реализован в технологии md.

<a name="redefine"></a>
### Переопределение блока

Переопределение — это изменение реализации блока путем
добавления ему новых особенностей на другом [уровне](#level).
