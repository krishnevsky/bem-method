...
* [Почему модификатор не может использоваться без указания его владельца](#bem-mod-usage)
* [Почему один модификатор не может применяться одновременно и к блоку, и к элементу](#mod-comb)
...
* Каждой БЭМ-сущности присваивается уникальное имя (смотри [соглашение по именованию](/naming/naming.ru.md)).
* Каждая БЭМ-сущность имеет CSS-класс, соответсвующий ее имени.

Уникальное имя и класс БЭМ-сущности позволяет создавать независимые CSS-правила, изменять и переопределять их. Это дает возможность повторно использовать блоки, перемещать по странице, вкладывать один блок в другой и использовать [миксы](/definitions/definitions.ru.md#mix).

Альтернативой классам могут служить data-атрибуты.
...
Гарантировать уникальность блока на странице нельзя. Блок поиска (`search`) долгое время использовался только один раз на странице.
...
Однако было решено продублировать его внизу страницы для удобства повторного поискового запроса.

Создавать правила, опираясь на id блока стало невозможным, в отличие от использования классов:
...
Использование селектора типа для определения блока не позволяет повторно применить блок на странице. Для этого необходимо разделить CSS-правила, сделать блоки независимыми. Используя правила, созданные для HTML-тега, сделать это невозможно. Чтобы решить проблему, необходимо добавить имя блока к классу.
